...
enum EditDiffer {
    ...
    /// Check that the replacement looks similar to the original (same word spelled differently,
    /// not a completely different word). Uses normalized Levenshtein distance.
    private static func isSimilar(_ lhs: String, _ rhs: String) -> Bool {
        let lowerA = lhs.lowercased()
        let lowerB = rhs.lowercased()
        let maxLen = max(lowerA.count, lowerB.count)
        guard maxLen > 0 else { return true }
        let dist = levenshtein(lowerA, lowerB)
